...
function index(props) {
  ...
  const onSearch = (action, item) => {
    ...
    let loaiCsyt = action === "loaiCsyt" ? item : state.loaiCsyt;
    let createdAt =
      action === "createdAt"
    ...
    let dmCoQuanQuanLyId =
      action === "dmCoQuanQuanLyId" ? item : state.dmCoQuanQuanLyId;
    let params = {
      page,
      size,
              ...
              s.data && s.data.length
                ? s.data.map((item, index) => {
                    return {
                      ...
                      col4: item.ten,
                      col5: item.loaiCsyt,
                      col6: item.coQuanQuanLyIds,
                      col7: item.dmTinhThanhPho && item.dmTinhThanhPho.ten,
                      col8: item.dmQuanHuyen && item.dmQuanHuyen.ten,
                      col9: item.soDienThoai,
                      col10: item.nguoiLienHe,
                      col11: new Date(item.createdAt).format("dd-MM-YYYY"),
                      col12: item,
                    };
                  })
                : [],
  ...
  const addReport = () => {
    ...
    hospitalProvider
      ...
      .then((s) => {
        ...
        a.href = s;
        a.download = `Danh mục CSYT  ${new Date().format(
          "ddMMyyyyHHmmss"
        )}.xls`;
        a.click();
  ...
  const onDeleteItem = (item) => {
    return new Promise((resolve, reject) => {
      confirm({
        title: "Xác nhận",
        content: `Bạn có muốn xóa cơ sở y tế ${item.ten}?`,
        okText: "Xóa",
        okType: "danger",
        cancelText: "Hủy",
        onOk() {
          hospitalProvider
            ...
            .then((s) => {
              if (s && s.code === 0) {
                snackbar.show("Xóa cơ sở y tế thành công", "success");
                onSearch();
              } else if (s && s.code === 1900) {
                snackbar.show("Cơ sở y tế đang được sử dụng!", "danger");
              } else {
                snackbar.show("Xóa cơ sở y tế không thành công", "danger");
              }
            })
            .catch((e) => {});
        },
  ...
  const editItem = (item) => {
    if (item) {
      props.history.push("/admin/hospital/edit/" + item.id);
    } else {
      props.history.push("/admin/hospital/create");
    }
  };
  const checkHospital = (data) => {
    let status = DataContants.listTypeHospital.filter((item) => {
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              title: (
                <div className="custome-header">
                  <div className="title-box">Mã bệnh viện</div>
            ...
            },
            {
              title: (
                <div className="custome-header">
                  <div className="title-box">Loại cơ sở y tế</div>
                  <div className="addition-box">
                    <Select
                      value={state.loaiCsyt}
                      onChange={(e) => {
                        onSearch("loaiCsyt", e);
                        setState({
                          loaiCsyt: e,
            ...
            },
            {
              title: (
                <div className="custome-header">
                  <div className="title-box">Cơ quan quản lý trực tiếp</div>
                  <div className="addition-box">
                    <Select
                      value={state.dmCoQuanQuanLyId}
                      onChange={(e) => {
                        onSearch("dmCoQuanQuanLyId", e);
                        setState({
                          dmCoQuanQuanLyId: e,
                        });
                      }}
                      ...
                      placeholder="Chọn cơ quan quản lý trực tiếp"
                    >
                      <Option value={""}>Tất cả</Option>
