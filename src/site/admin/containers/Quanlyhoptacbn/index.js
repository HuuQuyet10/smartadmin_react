...
function index(props) {
    ...
    const onDeleteItem = (item) => {
        return new Promise((resolve, reject) => {
            confirm({
              title: "Xác nhận",
              content: `Bạn có muốn xóa ${item.cooperativeBasis.name}?`,
              okText: "Xóa",
              okType: "danger",
              cancelText: "Hủy",
              onOk() {
                cosohoptacProvider
                  .delete(item.cooperativeBasis.id)
                  .then((s) => {
                    if (s && s.code === 0) {
    ...
    const editItem = (item) => {
        if(item) {
            props.history.push("/admin/quan-ly-hop-tac-co-so/chinh-sua/" + item.cooperativeBasis.id);
        } else {
            props.history.push("/admin/quan-ly-hop-tac-co-so/them-moi");
        }
    };
    ...
    const onSearch = (action, item) => {
        ...
        let name = action === "name" ? item : name;
        ...
        let createdUser = action === "createdUser" ? item : createdUser;
        cosohoptacProvider
          .search(
            page,
            size,
                  ...
                  s.data.data && s.data.data.length
                    ? s.data.data.map((item, index) => {
                        return {
                          ...
                          col2: item.cooperativeBasis.name,
                          col3: item.cooperativeBasis.link,
                          col4: new Date(item.cooperativeBasis.createdDate).format("dd-MM-YYYY"),
                          col5: item.cooperativeBasis.createdUser,
                          col6: item,
                        };
                      })
                    : [],
    ...
    return (
        <AdminPage
            ...
            header="Quản lý Cơ sở hợp tác"
            subheader="Danh sách Cơ sở hợp tác"
        >
            <Panel
                id="panel-list-site"
                title="DANH SÁCH CƠ SỞ HỢP TÁC"
                ...
                <Table
                    ...
                    columns={[
                        ...
                        },
                        {
                            title: (
                                <div className="custome-header">
                                    <div className="title-box">Cơ sở hợp tác</div>
